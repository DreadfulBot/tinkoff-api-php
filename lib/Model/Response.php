...
class Response implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['payment_url']) && (mb_strlen($this->container['payment_url']) > 100)) {
            $invalidProperties[] = "invalid value for 'payment_url', the character length must be smaller than or equal to 100.";
        }
    ...
    public function setTerminalKey($terminal_key)
    {
        if ((mb_strlen($terminal_key) > 20)) {
            throw new \InvalidArgumentException('invalid length for $terminal_key when calling Response., must be smaller than or equal to 20.');
        }
    ...
    public function setOrderId($order_id)
    {
        if ((mb_strlen($order_id) > 36)) {
            throw new \InvalidArgumentException('invalid length for $order_id when calling Response., must be smaller than or equal to 36.');
        }
    ...
    public function setStatus($status)
    {
        if ((mb_strlen($status) > 20)) {
            throw new \InvalidArgumentException('invalid length for $status when calling Response., must be smaller than or equal to 20.');
        }
    ...
    public function setPaymentId($payment_id)
    {
        if ((mb_strlen($payment_id) > 20)) {
            throw new \InvalidArgumentException('invalid length for $payment_id when calling Response., must be smaller than or equal to 20.');
        }
    ...
    public function setErrorCode($error_code)
    {
        if ((mb_strlen($error_code) > 20)) {
            throw new \InvalidArgumentException('invalid length for $error_code when calling Response., must be smaller than or equal to 20.');
        }
     ...
     * Sets payment_url
     *
     * @param string|null $payment_url Ссылка на платежную форму. Параметр возвращается только `для мерчантов без PCI DSS`.
     *
     * @return self
     */
    public function setPaymentUrl($payment_url)
    {
        if (!is_null($payment_url) && (mb_strlen($payment_url) > 100)) {
            throw new \InvalidArgumentException('invalid length for $payment_url when calling Response., must be smaller than or equal to 100.');
        }
    ...
    public function setMessage($message)
    {
        if (!is_null($message) && (mb_strlen($message) > 255)) {
            throw new \InvalidArgumentException('invalid length for $message when calling Response., must be smaller than or equal to 255.');
        }
