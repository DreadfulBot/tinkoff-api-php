...
/**
 * SupplierInfo Class Doc Comment
 *
 * @category Class
 * @description Данные поставщика платежного агента.  Обязателен, если передается значение &#x60;AgentSign&#x60; в объекте &#x60;AgentData&#x60;.
 * @package  RoxlTBankApi
 * @author   OpenAPI Generator team
 * @link     https://openapi-generator.tech
 * @implements \ArrayAccess<string, mixed>
 */
class SupplierInfo implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'SupplierInfo';
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['name']) && (mb_strlen($this->container['name']) > 239)) {
            $invalidProperties[] = "invalid value for 'name', the character length must be smaller than or equal to 239.";
        }
     ...
     * Sets phones
     *
     * @param string[]|null $phones `Тег ФФД: 1171`   Телефон поставщика, в формате `+{Ц}`. Атрибут обязателен, если передается значение `AgentSign` в объекте `AgentData`.
     *
     * @return self
     */
     ...
     * Sets name
     *
     * @param string|null $name `Тег ФФД: 1225`   Наименование поставщика. Атрибут обязателен, если передается значение `AgentSign`  в объекте `AgentData`. Внимание: в эти 239 символов включаются телефоны поставщика  + 4 символа на каждый телефон.   Например, если передано два телефона поставщика длиной 12 и 14 символов,  максимальная длина наименования поставщика будет  239 – (12 + 4) – (14 + 4) = 205 символов.
     *
     * @return self
     */
    public function setName($name)
    {
        if (!is_null($name) && (mb_strlen($name) > 239)) {
            throw new \InvalidArgumentException('invalid length for $name when calling SupplierInfo., must be smaller than or equal to 239.');
        }
     ...
     * Sets inn
     *
     * @param string|null $inn `Тег ФФД: 1226`   ИНН поставщика в формате `ЦЦЦЦЦЦЦЦЦЦ`. Атрибут обязателен, если передается значение `AgentSign`  в объекте `AgentData`.
     *
     * @return self
     */
    public function setInn($inn)
    {
        if (!is_null($inn) && (mb_strlen($inn) > 12)) {
            throw new \InvalidArgumentException('invalid length for $inn when calling SupplierInfo., must be smaller than or equal to 12.');
        }
        if (!is_null($inn) && (mb_strlen($inn) < 10)) {
            throw new \InvalidArgumentException('invalid length for $inn when calling SupplierInfo., must be bigger than or equal to 10.');
        }
