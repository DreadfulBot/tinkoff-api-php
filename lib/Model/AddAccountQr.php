...
/**
 * AddAccountQr Class Doc Comment
 *
 * @category Class
 * @package  TinkoffApi
 * @author   OpenAPI Generator team
 ...
 */
class AddAccountQr implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'AddAccountQr';
    ...
    public const DATA_TYPE_PAYLOAD = 'PAYLOAD';
    ...
    public function __construct(?array $data = null)
    {
        ...
        $this->setIfExists('data_type', $data ?? [], 'PAYLOAD');
    ...
    */
     ...
     * Sets description
     *
     * @param string $description Подробное описание деталей заказа.
     *
     * @return self
     */
     ...
     * Sets data_type
     *
     * @param string|null $data_type Тип возвращаемых данных: * `PAYLOAD` — в ответе возвращается только Payload. Значение по умолчанию. * `IMAGE` — в ответе возвращается SVG-изображение QR.
     *
     * @return self
     */
     ...
     * Sets bank_id
     *
     * @param string|null $bank_id Внутренний идентификатор банка, который выбран для оплаты. Cписок доступных `BankId` запрашивается методом - [GetQrBankList](#tag/SBP/operation/GetQrBankList). При передаче `BankId`, в ответе в параметре `Data` будет возвращен deeplink вместо функциональной платежной ссылки (payload).  Следует передавать `BankId` только для `DataType = 'PAYLOAD'` или `null`.
     *
     * @return self
     */
    ...
    public function setRedirectDueDate($redirect_due_date)
    {
        if (is_null($redirect_due_date)) {
            throw new \InvalidArgumentException('non-nullable redirect_due_date cannot be null');
        }
