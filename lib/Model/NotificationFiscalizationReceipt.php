<?php
/**
 * NotificationFiscalizationReceipt
 *
 * PHP version 7.4
 *
 * @category Class
 ...
 */
class NotificationFiscalizationReceipt implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'NotificationFiscalization_Receipt';
    ...
    */
     ...
     * Sets ffd_version
     *
     * @param string|null $ffd_version Версия ФФД. Возможные значения: * FfdVersion: `1.2`, * FfdVersion: `1.05`.  Версия ФФД по умолчанию — `1.05`.
     *
     * @return self
     */
     ...
     * Sets taxation
     *
     * @param string|null $taxation Система налогообложения. Возможные значения: * `osn` — общая СН; * `usn_income` — упрощенная СН (доходы); * `usn_income_outcome` — упрощенная СН (доходы минус расходы); * `envd` — единый налог на вмененный доход; * `esn` — единый сельскохозяйственный налог; * `patent` — патентная СН.
     *
     * @return self
     */
    ...
    {
        $allowedValues = $this->getTaxationAllowableValues();
        if (!is_null($taxation) && !in_array($taxation, $allowedValues, true)) {
     ...
     * Sets email
     *
     * @param string|null $email Электронная почта клиента. Должен быть заполнен, если не передано значение  в параметре `Phone`.
     *
     * @return self
     */
    public function setEmail($email)
    {
        if (!is_null($email) && (mb_strlen($email) > 64)) {
            throw new \InvalidArgumentException('invalid length for $email when calling NotificationFiscalizationReceipt., must be smaller than or equal to 64.');
        }
    ...
    public function setPhone($phone)
    {
        if (!is_null($phone) && (mb_strlen($phone) > 64)) {
            throw new \InvalidArgumentException('invalid length for $phone when calling NotificationFiscalizationReceipt., must be smaller than or equal to 64.');
        }
     ...
     * Sets customer_inn
     *
     * @param string|null $customer_inn ИНН клиента.
     *
     * @return self
     */
     ...
     * Gets items
     *
     * @return \RoxlTBankApi\Model\ItemsFFD105[]|null
     */
     ...
     * Sets items
     *
     * @param \RoxlTBankApi\Model\ItemsFFD105[]|null $items Массив позиций чека с информацией о товарах.
     *
     * @return self
     */
     ...
     * Sets payments
     *
     * @param \RoxlTBankApi\Model\Payments[]|null $payments Объект c информацией о видах суммы платежа. * Если объект не передан, будет автоматически указана итоговая сумма чека с видом оплаты  «Безналичный». * Если передан объект `receipt.Payments`, значение в `Electronic` должно быть равно итоговому значению Amount в методе **Init**. При этом сумма введенных значений по всем видам оплат, включая `Electronic`, должна быть равна сумме (`Amount`) всех товаров, переданных в объекте `receipt.Items`.
     *
     * @return self
     */
