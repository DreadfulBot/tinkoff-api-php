...
class Cancel implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    protected static $openAPITypes = [
        ...
        'ip' => 'string',
        'amount' => 'float',
        'receipt' => '\RoxlTBankApi\Model\CancelReceipt',
        'shops' => '\RoxlTBankApi\Model\ShopsCancel[]',
        'qr_member_id' => 'string',
        'route' => 'string',
        'source' => 'string',
    ...
    protected static $setters = [
        ...
        'receipt' => 'setReceipt',
        'shops' => 'setShops',
        'qr_member_id' => 'setQrMemberId',
        'route' => 'setRoute',
        'source' => 'setSource',
        'external_request_id' => 'setExternalRequestId'
    ];
    ...
    protected static $getters = [
        ...
        'receipt' => 'getReceipt',
        'shops' => 'getShops',
        'qr_member_id' => 'getQrMemberId',
        'route' => 'getRoute',
        'source' => 'getSource',
        'external_request_id' => 'getExternalRequestId'
    ];
    ...
    public const ROUTE_TCB = 'TCB';
    public const ROUTE_BNPL = 'BNPL';
    public const SOURCE_INSTALLMENT = 'installment';
    public const SOURCE_BNPL = 'BNPL';
    ...
    public function __construct(array $data = null)
    {
        ...
        $this->setIfExists('qr_member_id', $data ?? [], null);
        $this->setIfExists('route', $data ?? [], null);
        $this->setIfExists('source', $data ?? [], null);
        $this->setIfExists('external_request_id', $data ?? [], null);
    }
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['external_request_id']) && (mb_strlen($this->container['external_request_id']) > 256)) {
            $invalidProperties[] = "invalid value for 'external_request_id', the character length must be smaller than or equal to 256.";
        }
     ...
     * Sets payment_id
     *
     * @param string $payment_id Идентификатор платежа в системе Т‑Бизнес.
     *
     * @return self
     */
     ...
     * Sets token
     *
     * @param string $token Подпись запроса — хэш `SHA-256`.
     *
     * @return self
     */
     ...
     * Sets amount
     *
     * @param float|null $amount Сумма в копейках. Если не передан, используется `Amount`, переданный в методе **Init**.   При отмене статуса `NEW` поле `Amount` игнорируется, даже если оно заполнено. Отмена производится на полную сумму.
     *
     * @return self
     */
     ...
     * Gets receipt
     *
     * @return \RoxlTBankApi\Model\CancelReceipt|null
     */
     ...
     * Sets receipt
     *
     * @param \RoxlTBankApi\Model\CancelReceipt|null $receipt receipt
     *
     * @return self
     */
     ...
     * Gets shops
     *
     * @return \RoxlTBankApi\Model\ShopsCancel[]|null
     */
     ...
     * Sets shops
     *
     * @param \RoxlTBankApi\Model\ShopsCancel[]|null $shops Обязательный для маркетплейсов. JSON-объект с данными маркетплейса.
     *
     * @return self
     */
     ...
     * @return string|null
     */
    public function getQrMemberId()
    {
        return $this->container['qr_member_id'];
    }

    /**
     * Sets qr_member_id
     *
     * @param string|null $qr_member_id Код банка в классификации СБП, в который нужно выполнить возврат. Смотрите параметр `MemberId` методе [**QrMembersList**](#tag/Oplata-cherez-SBP/paths/~1QrMembersList/post).
     *
     * @return self
     */
    public function setQrMemberId($qr_member_id)
    {

        if (is_null($qr_member_id)) {
            throw new \InvalidArgumentException('non-nullable qr_member_id cannot be null');
        }

        $this->container['qr_member_id'] = $qr_member_id;
     ...
     * Sets route
     *
     * @param string|null $route Способ платежа.
     *
     * @return self
     */
     ...
     * Sets source
     *
     * @param string|null $source Источник платежа.
     *
     * @return self
     */
    ...
    /**
     * Gets external_request_id
     *
     * @return string|null
     */
    public function getExternalRequestId()
    {
        return $this->container['external_request_id'];
    }

    /**
     * Sets external_request_id
     *
     * @param string|null $external_request_id Идентификатор операции на стороне мерчанта. Параметр не работает для операций по СБП. Обязателен для операций «Долями» и в рассрочку.  * Если поле не передано или пустое (\"\"), запрос будет обработан без проверки ранее созданных возвратов. * Если поле заполнено, перед проведением возврата проверяется запрос на отмену с таким `ExternalRequestId`. * Если такой запрос уже есть, в ответе вернется текущее состояние платежной операции, если нет — платеж отменится. * Для операций «Долями» при заполнении параметра нужно генерировать значение в формате `UUID v4`. * Для операций в рассрочку при заполнении параметра нужно генерировать значение с типом `string` — ограничение 100 символов.
     *
     * @return self
     */
    public function setExternalRequestId($external_request_id)
    {
        if (!is_null($external_request_id) && (mb_strlen($external_request_id) > 256)) {
            throw new \InvalidArgumentException('invalid length for $external_request_id when calling Cancel., must be smaller than or equal to 256.');
        }


        if (is_null($external_request_id)) {
            throw new \InvalidArgumentException('non-nullable external_request_id cannot be null');
        }

        $this->container['external_request_id'] = $external_request_id;
