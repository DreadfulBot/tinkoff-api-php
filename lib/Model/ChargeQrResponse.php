...
/**
 * ChargeQrResponse Class Doc Comment
 *
 * @category Class
 * @package  RoxlTBankApi
 * @author   OpenAPI Generator team
 ...
 */
class ChargeQrResponse implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'ChargeQrResponse';
    ...
    */
    ...
    public function setTerminalKey($terminal_key)
    {
        if ((mb_strlen($terminal_key) > 20)) {
            throw new \InvalidArgumentException('invalid length for $terminal_key when calling ChargeQrResponse., must be smaller than or equal to 20.');
        }
    ...
    public function setOrderId($order_id)
    {
        if ((mb_strlen($order_id) > 36)) {
            throw new \InvalidArgumentException('invalid length for $order_id when calling ChargeQrResponse., must be smaller than or equal to 36.');
        }
     ...
     * Sets status
     *
     * @param string|null $status Статус платежа. Возвращается один из трех статусов: * `CONFIRMED` — если платеж выполнен; * `REJECTED` — если платеж не выполнен; * `FORM SHOWED` — если форма оплаты пока что только отображается, и клиент еще не успел провести оплату.
     *
     * @return self
     */
    public function setStatus($status)
    {
        ...
        if (!is_null($status) && (mb_strlen($status) > 20)) {
            throw new \InvalidArgumentException('invalid length for $status when calling ChargeQrResponse., must be smaller than or equal to 20.');
        }
    ...
    public function setPaymentId($payment_id)
    {
        if ((mb_strlen($payment_id) > 20)) {
            throw new \InvalidArgumentException('invalid length for $payment_id when calling ChargeQrResponse., must be smaller than or equal to 20.');
        }
    ...
    public function setErrorCode($error_code)
    {
        if ((mb_strlen($error_code) > 20)) {
            throw new \InvalidArgumentException('invalid length for $error_code when calling ChargeQrResponse., must be smaller than or equal to 20.');
        }
    ...
    public function setMessage($message)
    {
        if (!is_null($message) && (mb_strlen($message) > 255)) {
            throw new \InvalidArgumentException('invalid length for $message when calling ChargeQrResponse., must be smaller than or equal to 255.');
        }
     ...
     * Sets currency
     *
     * @param float|null $currency Код валюты по `ISO 4217`.
     *
     * @return self
     */
