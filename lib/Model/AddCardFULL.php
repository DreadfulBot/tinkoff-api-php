 ...
 */
class AddCardFULL implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'AddCard_FULL';
    ...
    public const CHECK_TYPE_NO = 'NO';
    public const CHECK_TYPE_HOLD = 'HOLD';
    public const CHECK_TYPE__3_DS = '3DS';
    public const CHECK_TYPE__3_DSHOLD = '3DSHOLD';
    ...
    public function getCheckTypeAllowableValues()
    {
        return [
            self::CHECK_TYPE_NO,
            self::CHECK_TYPE_HOLD,
            self::CHECK_TYPE__3_DS,
            self::CHECK_TYPE__3_DSHOLD,
        ];
    }
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['ip']) && (mb_strlen($this->container['ip']) > 40)) {
            $invalidProperties[] = "invalid value for 'ip', the character length must be smaller than or equal to 40.";
        }
    ...
    public function setCustomerKey($customer_key)
    {
        if ((mb_strlen($customer_key) > 36)) {
            throw new \InvalidArgumentException('invalid length for $customer_key when calling AddCardFULL., must be smaller than or equal to 36.');
        }
    ...
    {
        if (!is_null($ip) && (mb_strlen($ip) > 40)) {
            throw new \InvalidArgumentException('invalid length for $ip when calling AddCardFULL., must be smaller than or equal to 40.');
        }
     ...
     * Sets resident_state
     *
     * @param bool|null $resident_state Признак резидентности добавляемой карты: Возможные значения: * `true` — карта РФ, * `false` — карта не РФ, * `null` — не специфицируется, универсальная карта.
     *
     * @return self
     */
    public function setResidentState($resident_state)
    {
        ...
        if (is_null($resident_state)) {
            throw new \InvalidArgumentException('non-nullable resident_state cannot be null');
        }
