 ...
 */
class NotificationAddCard implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'NotificationAddCard';
    ...
    */
    ...
    public function setTerminalKey($terminal_key)
    {
        if (!is_null($terminal_key) && (mb_strlen($terminal_key) > 20)) {
            throw new \InvalidArgumentException('invalid length for $terminal_key when calling NotificationAddCard., must be smaller than or equal to 20.');
        }
    ...
    public function setCustomerKey($customer_key)
    {
        if (!is_null($customer_key) && (mb_strlen($customer_key) > 36)) {
            throw new \InvalidArgumentException('invalid length for $customer_key when calling NotificationAddCard., must be smaller than or equal to 36.');
        }
     ...
     * Sets request_key
     *
     * @param string|null $request_key Идентификатор запроса на привязку карты.
     *
     * @return self
     */
     ...
     * Sets status
     *
     * @param string|null $status Статус привязки карты. Возвращается один из двух статусов:   * `COMPLETED` — при одностадийной оплате;   * `REJECTED` — при двухстадийной оплате.
     *
     * @return self
     */
    public function setStatus($status)
    {
        ...
        if (!is_null($status) && (mb_strlen($status) > 20)) {
            throw new \InvalidArgumentException('invalid length for $status when calling NotificationAddCard., must be smaller than or equal to 20.');
        }
    ...
    public function setErrorCode($error_code)
    {
        if (!is_null($error_code) && (mb_strlen($error_code) > 20)) {
            throw new \InvalidArgumentException('invalid length for $error_code when calling NotificationAddCard., must be smaller than or equal to 20.');
        }
    ...
    {
        if (!is_null($rebill_id) && (mb_strlen($rebill_id) > 20)) {
            throw new \InvalidArgumentException('invalid length for $rebill_id when calling NotificationAddCard., must be smaller than or equal to 20.');
        }
