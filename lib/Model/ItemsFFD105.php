 ...
 */
class ItemsFFD105 implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function __construct(?array $data = null)
    {
        ...
        $this->setIfExists('payment_method', $data ?? [], 'full_payment');
        $this->setIfExists('payment_object', $data ?? [], 'commodity');
    ...
    */
     ...
     * Sets price
     *
     * @param float $price `Тег ФФД: 1078`   Цена в копейках.
     *
     * @return self
     */
     ...
     * Sets quantity
     *
     * @param float $quantity `Тег ФФД: 1023`   Количество или вес товара. Максимальное количество символов — 8, где целая часть — не больше 5 знаков, дробная — не больше 3 знаков для АТОЛ,  и 2 знаков для CloudPayments.
     *
     * @return self
     */
     ...
     * Sets payment_method
     *
     * @param string|null $payment_method `Тег ФФД: 1214`   Возможные значения: * `full_prepayment` — предоплата 100%, * `prepayment` — предоплата, * `advance` — аванс, * `full_payment` — полный расчет, * `partial_payment` — частичный расчет и кредит, * `credit` — передача в кредит, * `credit_payment` — оплата кредита.   Если значение не передано, по умолчанию в онлайн-кассу передается признак способа расчёта `full_payment`.
     *
     * @return self
     */
     ...
     * Sets tax
     *
     * @param string $tax `Тег ФФД: 1199`   Ставка НДС. Перечисление со значениями: * `none` — без НДС, * `vat0` — НДС по ставке 0%, * `vat5` — НДС по ставке 5%, * `vat7` — НДС по ставке 7%, * `vat10` — НДС по ставке 10%, * `vat20` — НДС по ставке 20%, * `vat105` — НДС чека по расчетной ставке 5/105, * `vat107` — НДС чека по расчетной ставке 7/107, * `vat110` — НДС чека по расчетной ставке 10/110, * `vat120` — НДС чека по расчетной ставке 20/120.
     *
     * @return self
     */
     ...
     * Sets shop_code
     *
     * @param string|null $shop_code Код магазина. Для параметра `ShopСode` нужно использовать значение параметра `Submerchant_ID`, который возвращается в ответн при регистрации магазинов через XML. Если XML не используется, передавать поле не нужно.
     *
     * @return self
     */
    public function setShopCode($shop_code)
    {
        if (is_null($shop_code)) {
            throw new \InvalidArgumentException('non-nullable shop_code cannot be null');
        }
