 ...
 */
class ChargeFULL implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'Charge_FULL';
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        if ($this->container['rebill_id'] === null) {
            $invalidProperties[] = "'rebill_id' can't be null";
        }
    ...
    public function setTerminalKey($terminal_key)
    {
        if ((mb_strlen($terminal_key) > 20)) {
            throw new \InvalidArgumentException('invalid length for $terminal_key when calling ChargeFULL., must be smaller than or equal to 20.');
        }
    ...
    public function setPaymentId($payment_id)
    {
        if ((mb_strlen($payment_id) > 20)) {
            throw new \InvalidArgumentException('invalid length for $payment_id when calling ChargeFULL., must be smaller than or equal to 20.');
        }
     ...
     * Sets rebill_id
     *
     * @param string $rebill_id Идентификатор рекуррентного платежа. Значение зависит от атрибутов:   * `OperationInitiatorType` в методе **init**,   * `Recurrent` в методе **Init**.  Подробнее — в описаниях [Рекуррентный платёж](#tag/Rekurrentnyj-platyozh) и [Инициализация платежа](#tag/Standartnyj-platyozh/paths/~1Init/post)
     *
     * @return self
     */
     ...
     * Sets send_email
     *
     * @param bool|null $send_email `true` — если клиент хочет получать уведомления на почту.
     *
     * @return self
     */
     ...
     * Sets info_email
     *
     * @param string|null $info_email Адрес почты клиента. Обязателен при передаче `SendEmail`.
     *
     * @return self
     */
